...
""" This script is a copy of extract_2D.py and is linked to a job script. It is used to help analyze larger runs made my simulation also complete with a job.
    Scripts with same name but larger number marked are also copies to run jobs simutaneously. 
    That said, for *this script only*, another layer of data binning is added for binning fine scan with less photons into coarser bins. Comment out if situation does not apply. (line 272 - 293)
"""

fname = os.path.join(
    '/home', 'fchiu', 'projects', 'def-blairt2k', 'fchiu', 'outputs', 'output_mPMT535.root'
)
...
### binning fine data into coarse bins ###
raw = pd.DataFrame({'x': pos_x, 'y': pos_y, 'z': pos_z, 'absorbed': ph_absorbed, 'total': ph_total})    # collect raw data
bin_width = 1.0                                                                                         # set bin size in mm
raw['x_bin'] = np.floor(raw['x']/bin_width)*bin_width                                                   # resize x data
raw['y_bin'] = np.floor(raw['y']/bin_width)*bin_width                                                   # resize y data

binned = raw.groupby(['x_bin', 'y_bin']).agg(                                                           # bin data into the new xy points
    z_cen = ('z', 'mean'),                                                                              # although it is also a set of position data, it is dependent on x and y positions it is also binned by the new xy positions
    ph_absorbed=('absorbed', 'sum'),
    ph_total = ('total', 'sum'),
    pt = ('absorbed', 'count'),                                                                         # photon count checking
).reset_index()

pos_x = binned['x_centre'] = binned['x_bin'] + bin_width/2                                              # take centre of bin as new points
pos_y = binned['y_centre'] = binned['y_bin'] + bin_width/2
ratios = binned['r'] = binned['ph_absorbed']/binned['ph_total']                                         # recalculate absorption ratios after binning absorption photons and total number of photons
z_cen = binned['z_cen']                                                                                 # rename binned data
ph_absorbed = binned['ph_absorbed']
ph_total = binned['ph_total']

expected_pt = (bin_width/0.1)**3
binned['full'] = binned['pt'] == expected_pt
...
with open("absorption_data.txt", "w") as f:
    ...
    for i in range(len(pos_x)):
        sim_row = (f"{pos_x[i]}, {pos_y[i]}, {z_cen[i]}, {cos[i]}, {ph_total[i]}, {ph_absorbed[i]}, {ratios[i]}, {errors[i]}")
...
with open("outliers.txt", "w") as f:
    f.write(header + "\n")
    for i in range(len(pos_x)):
        pos_row = (f"{pos_x[i]}, {pos_y[i]}, {z_cen[i]}")
...
plot = ax.scatter(pos_x, pos_y, z_cen, c=ratios, cmap='viridis', vmin=0, vmax=1, s=5, alpha=1.0, edgecolors='none')
...
plt.savefig('absorption_ratios_3D_top-down_job.png', dpi=300)
plt.show()
plt.close()
...
plt.colorbar(plot, label='Absorption Ratio', shrink=0.6)
# ax.view_init(elev=90, azim=-90)
plt.savefig('absorption_ratios_3D_job.png', dpi=300)
plt.show()
...
mask = np.sqrt((((pos_x)/45)**2 + (pos_y/45)**2)) < 1
...
ax.view_init(elev=90, azim=-90)
plt.savefig('absorption_spots_3D_job.png', dpi=300)
plt.show()

#####################################################################################

out_x_mask = [out_x[i] for i in range(len(out_y)) if mask[i]]
...
print("Total number of weird photons:", len(w_x))
...
plt.savefig('scattered_spots_3D_job.png', dpi=300)
plt.show()
plt.close()
...
plt.savefig('Reflected_spots_top-down_job.png', dpi=300)
plt.show()
...
ax.view_init(elev=90, azim=-90)
plt.colorbar(plot, label='Number of reflected photons', shrink=0.6)
plt.savefig('Weird_spots_top-down_job.png', dpi=300)
plt.show()
