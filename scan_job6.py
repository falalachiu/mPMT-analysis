...
fname = os.path.join(
    '/home', 'fchiu', 'projects', 'def-blairt2k', 'fchiu', 'outputs', 'output_mPMT518.root'
)
...
# valid_area = Ellipse(xy = (-26, 0), width = 70, height = 95, edgecolor = 'red', fill = False, alpha = 0.6, lw = 2)
...
plt.savefig('absorption_ratios_3D_top-down_job6.png', dpi=300)
plt.show()
plt.close()
...
plt.colorbar(plot, label='Absorption Ratio', shrink=0.6)
# ax.view_init(elev=90, azim=-90)
plt.savefig('absorption_ratios_3D_job6.png', dpi=300)
plt.show()
...
mask = np.sqrt((((pos_x+26)/35)**2 + (pos_y/47.5)**2)) < 1
...
ax.view_init(elev=90, azim=-90)
plt.savefig('absorption_spots_3D_job6.png', dpi=300)
plt.show()
...
plt.savefig('scattered_spots_3D_job6.png', dpi=300)
plt.show()
plt.close()
...
plt.savefig('Reflected_spots_top-down_job6.png', dpi=300)
plt.show()
...
ax.view_init(elev=90, azim=-90)
plt.colorbar(plot, label='Number of reflected photons', shrink=0.6)
plt.savefig('Weird_spots_top-down_job6.png', dpi=300)
plt.show()
...
np.savetxt("outliers_list_6.txt", save_data, fmt="%f", header="reflection photons position (x,y,z), scattered photon position (x,y,z)")
...
np.savetxt("outliers_counts_6.txt", counter, fmt="%f", header="scattered photons position (x,y,z, counts), reflected photon position (x,y,z, counts)")

angle = np.degrees(np.arctan(np.sqrt(w_x_v**2+w_y_v**2)/(300**2)))
weird_data = np.column_stack((angle, w_x_v, w_y_v, w_z_v, w_c))
np.savetxt("weird photons_6.txt", weird_data, fmt="%f", header="weird photons injection angle tilt, landing positions (x,y,z) , count")