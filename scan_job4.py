...
ax.set_box_aspect([0.9, 1, 1.2])
plt.colorbar(plot, label='Absorption Ratio', shrink=0.6)
ax.view_init(elev=90, azim=-90)
plt.savefig('absorption_ratios_3D_top-down_job4.png', dpi=300)
plt.show()
plt.close()
...
plt.colorbar(plot, label='Absorption Ratio', shrink=0.6)
# ax.view_init(elev=90, azim=-90)
plt.savefig('absorption_ratios_3D_job4.png', dpi=300)
plt.show()
...
mask = np.sqrt((((pos_x-100)/42.5)**2 + (pos_y/45)**2)) < 1
...
ax.view_init(elev=90, azim=-90)
plt.savefig('absorption_spots_3D_job4.png', dpi=300)
plt.show()
...
plt.savefig('scattered_spots_3D_job4.png', dpi=300)
plt.show()
plt.close()
...
plt.savefig('Reflected_spots_top-down_job4.png', dpi=300)
plt.show()
...
ax.view_init(elev=90, azim=-90)
plt.colorbar(plot, label='Number of reflected photons', shrink=0.6)
plt.savefig('Weird_spots_top-down_job4.png', dpi=300)
plt.show()
...
np.savetxt("outliers_list_4.txt", save_data, fmt="%f", header="reflection photons position (x,y,z), scattered photon position (x,y,z)")
...
np.savetxt("outliers_counts_4.txt", counter, fmt="%f", header="scattered photons position (x,y,z, counts), reflected photon position (x,y,z, counts)")

angle = np.degrees(np.arctan(np.sqrt(w_x_v**2+w_y_v**2)/(300**2)))
weird_data = np.column_stack((angle, w_x_v, w_y_v, w_z_v, w_c))
np.savetxt("weird photons_4.txt", weird_data, fmt="%f", header="weird photons injection angle tilt, landing positions (x,y,z) , count")